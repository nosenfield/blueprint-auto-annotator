#!/bin/bash
# Deploy Geometric Conversion Lambda v1
...
ECR_REPO_NAME="geometric-conversion-v1"
LAMBDA_FUNCTION_NAME="geometric-conversion-v1"
...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}Geometric Conversion Lambda v1 Deployment${NC}"
echo -e "${GREEN}========================================${NC}"
echo ""
...
cd "$BACKEND_DIR"
docker buildx build --platform linux/amd64 -f lambda-geometric-conversion-v1/Dockerfile -t $ECR_REPO_NAME:latest --load .
...
if aws lambda get-function --function-name $LAMBDA_FUNCTION_NAME --region $AWS_REGION 2>/dev/null; then
    ...
    aws lambda create-function \
        ...
        --memory-size 2048 \
        --region $AWS_REGION \
        --description "Geometric conversion Lambda v1 - Converts wall detections to room polygons"
